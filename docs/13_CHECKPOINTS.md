...
| Phase 7            | Monetization                           | In progress        | `checkpoint.markAsBilled` action records payments. `Payment` and `PlanLimit` Prisma models added. `plan-core` package with `plan.checkEntitlement`, `plan.getPlan`, `plan.updatePlan` actions. 54 registered actions (LOW: 19, MEDIUM: 15, HIGH: 16). `pnpm docs:check` passes. | Add `checkpoint.recordPayment` action for external payment provider callback. Wire entitlement checks into the create actions (gates). Add billing API routes and UI. |
