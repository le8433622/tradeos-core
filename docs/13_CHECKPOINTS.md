...
21. **Resolve staging DB drift** — Staging Supabase `ulnjanlaehfmxurreibj` has only `_prisma_migrations` with 1 broken migration. Need to reset schema or create new staging project before live walkthrough.
22. **Choose Workspace page** — Build post-login page for users with >1 org
23. **#124 P0: Run first external buyer pain-solving case** — buyer `earthkingdomuniverse@gmail.com` ✅ signed up via magic link. Full flow rehearsed on staging.
