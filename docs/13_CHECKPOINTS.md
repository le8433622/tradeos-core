...
| P0 Evidence Intake UI + Draft Case Builder         | ✅ DONE         | `POST /api/evidence/parse` + `POST /api/evidence/items` + `EvidenceIntake` component (paste → parse → approve → save). `GET /api/evidence/build-draft` + `POST /api/evidence/drafts` + `/evidence` Evidence Inbox page (unattached evidence → Build Draft → edit → create SourcingRun DRAFT). Tenant safety fixed (org-scoped queries).                                                                             |
| P0 Global Evidence Intake on /evidence             | ✅ DONE         | `POST /api/evidence/items` accepts optional `sourcingRunId`. `EvidenceIntake` component with optional `sourcingRunId` prop. `/evidence` page has `<EvidenceIntake />` for paste → parse → save unattached evidence. Commits: `6c7cafb`.                                                                                                                                                                             |
| P0 Pain Detector from ParsedEvidence               | ✅ DONE         | `PainDetectorResult` with 10 pain flags, 6 dependency flags, `suggestedReason`, `suggestedNextStep`. `detectPain()` in `packages/evidence-core/src/pain-detector.ts`. `CURRENT_PRICE_UNKNOWN` and `ORIGIN_PRICE_UNKNOWN` separated correctly. 60 tests. Commits: `e1c5a48`, `5e9a309`.                                                                                                                              |
| P0 Operator Review Workflow                        | ✅ DONE         | `POST /api/evidence/drafts` enforces gates server-side: `NEEDS_MORE_EVIDENCE` requires `overrideReason`, `NEEDS_SUPPLIER_IDENTITY` requires `supplierName` or `overrideReason`. `reviewState` stored in metadata (`PROOF_PENDING`, `SUPPLIER_IDENTITY_PENDING`, `WAIT`). UI shows decision gate banner, override textarea, button per step. Commits: `a76c197`, `e6cdd39`.                                          |
| P0 Assisted Flow QA (integration tests)            | ✅ DONE         | 11 integration tests covering all 5 QA cases. 20/20 passed on local temp PostgreSQL. Bugs found & fixed: `createSourcingRunSchema` missing metadata keys (`suggestedNextStep`, `overrideReason`, `reviewState`, `requiredProof`, `supplierProvided`) added. Permission error message mismatch fixed (`ROLE_NOT_ALLOWED`). Org plan set to ENTERPRISE in tests to avoid FREE-tier entitlement cap (3 sourcing runs). |
| Staging DB drift (Supabase `ulnjanlaehfmxurreibj`) | 🔴 UNUSABLE     | Only `_prisma_migrations` table exists with 1 broken migration `20260522_deal_orgid` (references removed `Deal` table). No application tables. Cannot run live staging walkthrough until schema is rebuilt.                                                                                                                                                                                                         |
| Staging DB rebuild                                 | ✅ DONE         | `DROP SCHEMA public CASCADE` + `prisma db push` → 40 app tables created. `_prisma_migrations` no longer exists (no migration history). Org + OWNER user + roles/permissions seeded.                                                                                                                                                                                                                                 |
| Staging integration tests                          | ✅ PASS (20/20) | Assisted flow + action flow tests pass on staging DB.                                                                                                                                                                                                                                                                                                                                                               |
| Live staging /evidence walkthrough                 | ✅ PASS         | TC1: Complete quote → parse → save unattached → Build Draft (REQUEST_MORE_EVIDENCE) → Create as PROOF_PENDING → metadata verified (reviewState, requiredProof, pain/dependency flags, suggestedNextStep). TC2: Weak quote → NEEDS_MORE_EVIDENCE → create without override → 400. TC3: Weak quote + override → create → metadata.overrideReason saved.                                                               |
...
16. ~~**P0: Global Evidence Intake on /evidence**~~ ✅ DONE — `/evidence` page has paste → parse → save unattached evidence → Build Draft flow
17. ~~**P0: Pain Detector from ParsedEvidence**~~ ✅ DONE — `detectPain()` with correct CURRENT_PRICE_UNKNOWN / ORIGIN_PRICE_UNKNOWN separation, 60 tests, integrated into build-draft route
18. ~~**P0: Operator Review Workflow**~~ ✅ DONE — server-enforced decision gates per suggestedNextStep, override reason required for weak/supplier-missing cases, reviewState in metadata
19. ~~**P0: Assisted Flow QA**~~ ✅ DONE — 11 integration tests covering all 5 gate scenarios, cross-tenant isolation, permission checks
20. **Fix `createSourcingRunSchema` metadata keys** — ✅ DONE — added `suggestedNextStep`, `overrideReason`, `reviewState`, `requiredProof`, `supplierProvided` to `packages/sourcing-core/src/index.ts` Zod schema
21. **Resolve staging DB drift** — ✅ DONE — Rebuilt staging `ulnjanlaehfmxurreibj` schema (DROP SCHEMA public CASCADE + db push), seeded org/user/permissions, 20/20 integration tests pass, live walkthrough verified all 3 TC cases.
22. **Choose Workspace page** — Build post-login page for users with >1 org
23. **#124 P0: Run first external buyer pain-solving case** — buyer `earthkingdomuniverse@gmail.com` ✅ signed up via magic link. Assisted flow verified on staging. Ready for buyer case.
