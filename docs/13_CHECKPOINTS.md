...
| P0 Assisted Flow QA (integration tests)    | ✅ DONE | 11 integration tests in `packages/integration-tests/src/assisted-flow.test.ts` covering all 5 QA cases: complete quote, weak quote, weak+override, missing supplier+supplierName/override, missing landed cost. Cross-tenant isolation and permission checks included. Commit: pending.                                                                                    |
...
20. **Choose Workspace page** — Build post-login page for users with >1 org
21. **#124 P0: Run first external buyer pain-solving case** — buyer `earthkingdomuniverse@gmail.com` ✅ signed up via magic link. Full flow rehearsed on staging.
